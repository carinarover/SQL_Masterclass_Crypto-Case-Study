...
-- The cumulative and the yearly quantity is exactly the same! This is because our UPDATE step only takes into account a single row at a time, 
-- which is exactly what we must not do with our window functions! We will need to create an additional temp table with our cumulative sum instead!
DROP TABLE IF EXISTS temp_cumulative_portfolio_base;
CREATE TABLE temp_cumulative_portfolio_base AS SELECT
  first_name,
  region,
  year_end,
  ticker,
